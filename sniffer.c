...
void print_ipv4(char*, char*, const struct ether_header *ethernet_header, const struct pcap_pkthdr *header );
...
void handle_packet(u_char *args, const struct pcap_pkthdr *header, const u_char *packet)
{
	...
	case ETHERTYPE_IP:
		...
		switch(ipv4_header->ip_p){
			...
			print_ipv4(sourIP4, destIP4, ethernet_header, header);
			...
			printf("Payload: (%d bytes)", dataLength);
			printf("\n");
...
{    
    ...
    *size += tcp_header->doff*4;
    ...
    printf("protocol: TCP | ");
    printf("Src port: %d | ", sourPort);
    printf("Dst port: %d | ", destPort);
...
void print_ipv4(char *source, char *dest, const struct ether_header *ethernet_header, const struct pcap_pkthdr *header)
{
   char time[1000];
   struct tm * p = localtime((const time_t*)&header->ts.tv_sec);
   strftime(time, 1000, "%c", p);
    ...
    printf("Src MAC: %17s | ", mac_ntoa(ethernet_header->ether_shost));
    printf("Dst MAC: %17s | ", mac_ntoa(ethernet_header->ether_dhost));
